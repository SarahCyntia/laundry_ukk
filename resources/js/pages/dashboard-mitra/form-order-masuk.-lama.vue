...
async function loadJenisLayanan() {
    try {
        const res = await axios.get("/jenis-layanan"); // sesuaikan route kamu
...
//         .then(({ data }) => {
//             const d = data.data; // ← object order yang asli
...
//             };
//         })
//         .catch(() => toast.error("Gagal mengambil data order"))
//         .finally(() => {
...
//             order.value.harga_per_kg = res.data.harga_per_kg;
//         })
//         .catch(() => {
//             toast.error("Gagal mengambil harga layanan");
//             order.value.harga_per_kg = 0;
//         });
// }
