...
import Form from "./formlayanan.vue";
import { createColumnHelper } from "@tanstack/vue-table";
import type { jenislayanan } from "@/types";
import axios from "axios";
import Swal from "sweetalert2";  

const column = createColumnHelper<jenislayanan>();
...
// hapus biasa
const { delete: deleteJenisLayanan } = useDelete({
    onSuccess: () => refresh(),
});

// hapus permanen (sampah)
const { delete: forceDeleteJenisLayanan } = useDelete({
    onSuccess: () => trashRef.value.refetch(),
});

// restore data
// const { post: restoreJenisLayanan } = useDelete({
//     onSuccess: () => trashRef.value.refetch(),
// });

// const restoreJenisLayanan = (url: string) => {
//     axios.post(url).then(() => {
...
const columns = [
    ...
    column.accessor("nama_layanan", { header: "Nama Layanan" }),
                ...
                "button",
                {
                    ...
                    onClick: () => {
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                deleteJenisLayanan(`/jenis_layanan/${cell.getValue()}`)
                                    .then(() => {
                                        trashRef.value.refetch(); // refresh tabel sampah
                                    });
                            }
                        });
                    },
        ...
        cell: (cell) =>
            h("div", { class: "d-flex gap-2" }, [
                h("button", {
                    ...
                    title: "Restore",
                    onClick: () =>
                        restoreJenisLayanan(`/jenis_layanan/${cell.getValue()}/restore`),
                }, h("i", { class: "la la-undo fs-2" })),
                h("button", {
                    class: "btn btn-sm btn-icon btn-danger",
                    title: "Hapus Permanen",
                    onClick: () =>
                        forceDeleteJenisLayanan(`/jenis_layanan/${cell.getValue()}/force-delete`),
                }, h("i", { class: "la la-trash fs-2" })),
            ]),
    }),
];
...
<template>
    ...
    <div class="card mb-10">
        <div class="card-header align-items-center">
            <h2 class="mb-0">List Jenis Layanan</h2>
    ...
    <div class="card border-danger">
        <div class="card-header align-items-center">
            <h2 class="mb-0 text-danger">
                Jenis Layanan Trash <small>(data yang telah dihapus)</small>
            </h2>
        </div>
        <div class="card-body">
            <paginate
                ref="trashRef"
                id="table-jenis_layanan_trash"
                url="/jenis_layanan/trash"
