...
import Form from "./formprioritas.vue";
import { createColumnHelper } from "@tanstack/vue-table";
import type { layananprioritas } from "@/types";
import axios from "axios";
import Swal from "sweetalert2";  

const column = createColumnHelper<layananprioritas>();
...
// hapus biasa
const { delete: deleteLayananPrioritas } = useDelete({
    onSuccess: () => refresh(),
});

// hapus permanen (sampah)
const { delete: forceDeleteLayananPrioritas } = useDelete({
...
const columns = [
    ...
    column.accessor("prioritas",     { header: "Prioritas" }),
    column.accessor("id", {
        header: "Aksi",
        cell: (cell) =>
            h("div", { class: "d-flex gap-2" }, [
                ...
                h("button", {
                    ...
                    onClick: () =>{
                        ...
                        deleteLayananPrioritas(`/layanan_prioritas/${cell.getValue()}`)
                        trashRef.value.refetch()
                    }
...
const trashColumns = [
    ...
    column.accessor("updated_at",{ header: "Updated At" }),
    column.accessor("id", {
        header: "Aksi",
        cell: (cell) =>
            h("div", { class: "d-flex gap-2" }, [
                h("button", {
                    ...
                    title: "Restore",
                    onClick: () =>
                        restoreJenis(`/layanan_prioritas/${cell.getValue()}/restore`),
                }, h("i", { class: "la la-undo fs-2" })),
                h("button", {
                    class: "btn btn-sm btn-icon btn-danger",
                    title: "Hapus Permanen",
                    onClick: () =>
                        forceDeleteLayananPrioritas(`/layanan_prioritas/${cell.getValue()}/force-delete`),
                }, h("i", { class: "la la-trash fs-2" })),
            ]),
    }),
];
...
<template>
    ...
    <div class="card mb-10">
        ...
        <div class="card-body">
            <paginate
                ref="paginateRef"
                id="table-layanan_prioritas"
                url="/layanan_prioritas"
                :columns="columns"
            />
        </div>
    </div>
    ...
    <div class="card border-danger">
        ...
        <div class="card-body">
            <paginate
                ref="trashRef"
                id="table-layanan_prioritas_trash"
                url="/layanan_prioritas/trash"
